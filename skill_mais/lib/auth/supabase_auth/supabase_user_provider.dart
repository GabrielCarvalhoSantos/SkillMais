...
class SkillMaisSupabaseUser extends BaseAuthUser {
  SkillMaisSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> skillMaisSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SkillMaisSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
